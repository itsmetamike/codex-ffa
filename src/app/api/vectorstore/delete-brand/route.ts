...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.log(`[DELETE BRAND API] Deleting all files for brand: ${brand}`);
    ...
    console.log(`[DELETE BRAND API] Found ${fileIds.length} files to delete`);

    // Delete all files from OpenAI
    const deletePromises = fileIds.map(async (fileId) => {
      try {
        await client.files.delete(fileId);
        console.log(`[DELETE BRAND API] Deleted file: ${fileId}`);
      } catch (error) {
        console.error(`[DELETE BRAND API] Error deleting file ${fileId}:`, error);
      }
    });
    ...
    try {
      ...
      console.log(`[DELETE BRAND API] Deleted vector store: ${vectorStore.vectorStoreId}`);
    } catch (error) {
      console.error(`[DELETE BRAND API] Error deleting vector store:`, error);
    }
    ...
    console.log(`[DELETE BRAND API] Successfully deleted all data for brand: ${brand}`);
